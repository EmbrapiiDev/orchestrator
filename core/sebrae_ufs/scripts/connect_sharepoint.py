...
SHAREPOINT_DRIVE_NAME_SEBRAE = os.getenv("SHAREPOINT_DRIVE_NAME_SEBRAE")
...
class SharepointClient:
    def __init__(self, sebrae = False):
        ...
        # 🔽 Pega o site
        self.site = self._get_site(sebrae)
    ...
    def _get_site(self, sebrae):
        if sebrae:
            url = f"https://graph.microsoft.com/v1.0/sites/{SHAREPOINT_DOMAIN}:/sites/{SHAREPOINT_SITE_NAME_SEBRAE}"
        else:
    ...
    def download_file_from_other_site(self, site_url, doc_library, file_path, output_path):
        """
        Baixa arquivo de outro site do SharePoint, usando apenas o `site_url`.
        site_url: 'embrapii.sharepoint.com:/sites/NOME_DO_SITE'
        doc_library: nome da biblioteca de documentos (ex: 'Shared Documents')
        file_path: caminho do arquivo dentro da doc_library
        output_path: caminho local para salvar o arquivo
        """
        try:
            # Corrigir e pegar o site ID correto
            url_site = f"https://graph.microsoft.com/v1.0/sites/{site_url}?$select=id"
            res_site = requests.get(url_site, headers=self.headers)
            res_site.raise_for_status()
            site_id = res_site.json()["id"]
            print(f"📄 site_id (puro): {site_id}")

            # Pegar drive ID da doc_library
            url_drives = f"https://graph.microsoft.com/v1.0/sites/{site_id}/drives"
            res_drives = requests.get(url_drives, headers=self.headers)
            res_drives.raise_for_status()

            drives = res_drives.json()["value"]
            drive = next((d for d in drives if d["name"] == doc_library), None)
            if not drive:
                raise Exception(f"Drive '{doc_library}' não encontrado.")
            drive_id = drive["id"]
            print(f"📁 drive_id: {drive_id}")

            # Fazer download do arquivo
            url_download = f"https://graph.microsoft.com/v1.0/drives/{drive_id}/root:/{file_path}:/content"
            res_file = requests.get(url_download, headers=self.headers, stream=True)
    ...
    def listar_drives_site(self, site_id):
